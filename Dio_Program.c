...
{
	// Input Validation
	if ( (A_portID <= DIO_PORTD) && (A_pinID <= PINSEVEN) && (A_pinDirection <= DIO_OUTPUT) )
	{
		switch (A_portID)
		{
		case DIO_PORTA:
...
{
	// Input Validation
	if ( (A_portID <= DIO_PORTD) && (A_pinID <= PINSEVEN) && (A_pinValue <= DIO_SET) )
	{
		switch (A_portID)
		{
		case DIO_PORTA:
...
DIO_VALUE_e MDIO_getPinValue(DIO_PORT_e A_portID, DIO_PIN_e A_pinID)
{
	...
	// Input Validation
	if ( (A_portID <= DIO_PORTD) && (A_pinID <= PINSEVEN) )
	{
		switch (A_portID)
		{
		case DIO_PORTA:
